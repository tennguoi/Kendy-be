...
class OrderRaceConditionIntegrationTest {
    ...
    private Long insertBankTransaction(String depositCode, BigDecimal amount, String referenceCode) {
        try {
            ...
            var sepayIdField = BankTransaction.class.getDeclaredField("sepayId");
            sepayIdField.setAccessible(true);
            sepayIdField.set(transaction, Math.abs(referenceCode.hashCode()) + 10_000L);
            var gatewayField = BankTransaction.class.getDeclaredField("gateway");
            gatewayField.setAccessible(true);
            gatewayField.set(transaction, "MBBank");
            var bankNameField = BankTransaction.class.getDeclaredField("bankName");
            bankNameField.setAccessible(true);
            bankNameField.set(transaction, "MBBank");
            var accountNumberField = BankTransaction.class.getDeclaredField("accountNumber");
            accountNumberField.setAccessible(true);
            accountNumberField.set(transaction, "0123456789");
            var transactionDateField = BankTransaction.class.getDeclaredField("transactionDate");
            transactionDateField.setAccessible(true);
            transactionDateField.set(transaction, Instant.now());
            var transferTypeField = BankTransaction.class.getDeclaredField("transferType");
            transferTypeField.setAccessible(true);
            transferTypeField.set(transaction, "IN");
            var transferAmountField = BankTransaction.class.getDeclaredField("transferAmount");
            transferAmountField.setAccessible(true);
            transferAmountField.set(transaction, amount);
            var accumulatedField = BankTransaction.class.getDeclaredField("accumulated");
            accumulatedField.setAccessible(true);
            accumulatedField.set(transaction, amount);
            var codeField = BankTransaction.class.getDeclaredField("code");
            codeField.setAccessible(true);
            codeField.set(transaction, depositCode);
            var contentField = BankTransaction.class.getDeclaredField("content");
            contentField.setAccessible(true);
            contentField.set(transaction, "Thanh toan " + depositCode);
            var referenceCodeField = BankTransaction.class.getDeclaredField("referenceCode");
            referenceCodeField.setAccessible(true);
            referenceCodeField.set(transaction, referenceCode);
            var statusField = BankTransaction.class.getDeclaredField("status");
            statusField.setAccessible(true);
            statusField.set(transaction, BankTransactionStatus.NEW);
            var rawPayloadField = BankTransaction.class.getDeclaredField("rawPayload");
            rawPayloadField.setAccessible(true);
            rawPayloadField.set(transaction, "{}");
            var receivedAtField = BankTransaction.class.getDeclaredField("receivedAt");
            receivedAtField.setAccessible(true);
            receivedAtField.set(transaction, Instant.now());
